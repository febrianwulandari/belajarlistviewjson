...
public final class GempaUtils {
    ...
    public static ArrayList<Gempa> extractGempa() {
        ArrayList<Gempa> gempas = new ArrayList<>();
            // jika ada try harus ada catch
        try {
            JSONObject baseJsonResponse = new JSONObject(SAMPLE_JSON_RESPONSE);

            JSONArray gempaArray = baseJsonResponse.getJSONArray("features");
            // dari 0 sampai panjangnya array
            for (int i = 0; i < gempaArray.length(); i++) {
                JSONObject currentGempa = gempaArray.getJSONObject(i);

                JSONObject properties = currentGempa.getJSONObject("properties");
                ...
                Gempa gempa = new Gempa(magnitude, location, time, url);
                    gempas.add(gempa);
                }

        } catch (JSONException e) {
          Log.e("GempaUtils", "Masalah yang terjadi pada JSON Gempa",e);
            }
